...
核心脚本：`build_feasible_space.py`  
主配置：`config.feasible_space.json`
...
```powershell
python build_feasible_space.py --config config.feasible_space.json
```
...
- 地图与报告：
  ...
  - `substation_preprocessed.pcd`
- 禁飞与可通行叠加：
  - `overlay_cloud_nofly.png`
  - `cloud_nofly_rotate.gif`
  - `overlay_compare_nofly_free.png`
  - `cloud_nofly_free_compare_rotate.gif`
- 分类结果（按设备类型）：
  - `device_types_overview.png`
  - `device_types_rotate.gif`
- 可通行空域多视角：
  - `overlay_free_bands.png`（近地/中低空/全体积）
  - `cloud_free_bands_rotate.gif`
  - `free_space_layer_scan.gif`（俯视逐层扫描）
  - `free_space_layer_scan_iso.gif`（正等轴测逐层扫描）
- 扫参与敏感性分析：
  - `sweep_maps/sweep_summary.json`
  - `sweep_maps/sweep_summary.csv`
  - `sweep_maps/sweep_free_space_compare.png`
  - `sweep_maps/station_grid_map_dynamic_*.npz`
  - `sweep_maps/station_grid_map_static_*.npz`
...
## 7. 配置要点（`config.feasible_space.json`）
